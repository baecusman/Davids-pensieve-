...
interface DigestMonthSectionProps {
  month: string
  summary: string
...
export default function DigestMonthSection({
  month,
  summary,
  isCurrentMonth = false,
  children,
  itemCount,
}: DigestMonthSectionProps) {
  ...
  const shouldScroll = itemCount > 6
    ...
    <div className="border border-gray-200 rounded-lg bg-white mb-2 overflow-hidden">
      <button
        ...
        className={`w-full px-6 py-3 flex items-center justify-between hover:bg-gray-50 transition-all duration-200 ${
          isCurrentMonth ? "sticky top-0 z-10" : ""
        } ${isSticky && isCurrentMonth ? "bg-white/95 backdrop-blur-sm border-b border-gray-200 shadow-sm" : ""}`}
      >
      ...
      >
        <div className="px-6 pb-4 border-t border-gray-100">
          ...
          {/* Articles */}
          <div className={shouldScroll ? "max-h-[500px] overflow-y-auto pr-2" : ""}>{children}</div>
